...
@php
    $nama = '';
    $sql = $kategori::find(request()->parent);
    if ($sql) {
        $nama = $sql->nama;
    }

@endphp
<form action="{{ route('kategori.store') }}" onsubmit="return false" method="post" id="form-action">
    @csrf
    <input type="hidden" name="ID" value="{{ request()->parent }}">
    <div class="modal-body">
        <x-forms.form-input label="Nama" type="text" name="nama" placeholder="Kategori" :value="$nama" />
    </div>
