...
<form action="{{ route('user.store') }}" onsubmit="return false" method="post" id="form-action">
    ...
    <div class="modal-body">
        <x-forms.form-input label="Nama" type="text" name="name" placeholder="Tulis nama" :value="old('nama')" />
        <x-forms.form-input label="Email" type="email" name="email" placeholder="Tulis email" :value="old('email')" />
        <x-forms.form-input label="Password" type="password" name="password" placeholder="Tulis password"
            :value="old('password')" />
        <div class="mb-3">
            <label class="form-label">Role</label>
            <select class="form-control" name="role" id="role">
                {!! OptionCreate($roleName, $roleName, '') !!}
            </select>
        </div>
        <div class="mb-3">
